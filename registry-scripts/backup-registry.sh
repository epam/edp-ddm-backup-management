...
declare -a openshift_registry_resources=("service" "gerrit" "jenkins" "codebase" "jenkinsauthorizationrolemapping" "postgrescluster")
...
for object in `oc get crd --no-headers -o custom-columns="NAME:.metadata.name" | grep keycloak`;
do
  kind=${object%%.*}
  if [[ ! -z $(oc get ${kind} -n ${registry_name} --no-headers -o custom-columns="NAME:.metadata.name") ]]; then
    ...
    for resource_name in $(oc get ${kind} -n ${registry_name} --no-headers -o custom-columns="NAME:.metadata.name")
    do
...
for resource_kind in "${openshift_registry_resources[@]}"
do
    resource_folder="${resource_folder}/${resource_kind}"
    mkdir -p "${resource_folder}"
    for name in $(oc get "${resource_kind}" -n "${registry_name}" --no-headers -o custom-columns="NAME:.metadata.name")
    do
